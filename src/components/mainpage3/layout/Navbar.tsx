...
/**
 * Navbar — 1:1 port of /mainpage2 Navbar.
 * Smart sticky (transparent over hero → dark plate after, hide on scroll-down,
 * show on scroll-up), mega-dropdowns (Moving / Locations / About Us) with image
 * CTA + 2-col link list, glass phone CTA, "Get a quote" pill, full-screen mobile menu.
 */
          ...
          >
            ...
            <nav className="hidden lg:flex items-center gap-1 h-full">
              {navLinks.map((link) => {
                ...
                const labelContent = (
                  <>
                    <span className="pill-label-sizer" aria-hidden="true">
                      {link.label}{hasDropdown && chevron}
                    </span>
                ...
                return (
                  <Link key={link.href} href={link.href} className={`pill-nav-link ${NAV_FONT}`}>
                    {labelContent}
                  </Link>
                );
              })}
            ...
            >
              <span className="glass-phone-badge grid place-items-center w-10 h-10 rounded-full bg-accent text-accent-text">
                <svg className="glass-phone-icon" width="16" height="16" viewBox="0 0 24 24"
                    ...
                    {image && (
                      ...
                      >
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-4 flex items-center justify-between text-white z-10">
                          <span className="text-[1.1875rem] font-medium leading-[1.25] tracking-[-0.04em]">
                            {image.label}
                          </span>
                      ...
                      <ul className="grid grid-cols-2 gap-x-6 gap-y-0.5">
                        {allLinks.map((l, idx) => (
                          <motion.li
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.4, delay: idx * 0.025, ease: EASE }}
                          >
                            <Link
                              href={l.href}
                              onClick={() => setOpenDropdown(null)}
              ...
              <nav className="flex flex-col gap-2 flex-1">
                {navLinks.map((link, i) => (
                  ...
                  >
                    <Link
                      href={link.href}
                      className="block text-3xl font-bold text-white hover:text-accent transition-colors py-3 border-b border-white/10"
